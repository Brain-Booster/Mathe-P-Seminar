      ...
      xhr.onload = () => {
        if (xhr.status === 200) {
          try {
            ...
            if (response.success && response.modelPath) {
              // Call the callback with the uploaded model path
              if (onModelUploaded) {
                onModelUploaded(response.modelPath);
              }
              setProgress(100);
              ...
              setError('Fehler beim Hochladen des Models');
              setIsUploading(false);
            }
          } catch (e) {
          ...
          try {
            const response = JSON.parse(xhr.responseText);
            setError(response.error || 'Fehler beim Hochladen');
          } catch (e) {
