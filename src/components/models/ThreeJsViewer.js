...
export default function ThreeJsViewer({ modelPath, width = '100%', height = '100%' }) {
  ...
  useEffect(() => {
    ...
    try {
      renderer.outputEncoding = THREE.sRGBEncoding || THREE.SRGBColorSpace; // Handle THREE.js version differences
    } catch (err) {
      console.warn('ThreeJsViewer: Issue with output encoding:', err);
    }
