...
import { getClientProjects, getClientProjectsSync, updateServerProjects } from '../../models/models'; // Fix models import path
import { getClientTeamMembers, getClientTeamMembersSync } from '../../models/team';
import { updateTeamMembers } from '../../models/team';
...
export default function AdminProjects() {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(async () => {
      ...
      if (activeTab === 'edit') {
        ...
        await updateServerProjects(updatedProjects);
  ...
  const handleDeleteProject = (id) => {
    if (confirm("Sind Sie sicher, dass Sie dieses Projekt löschen möchten?")) {
      ...
      // Then update on the server
      updateServerProjects(updatedProjects)
        .then(() => {
  ...
  const handleEditSectionFileChange = async (e) => {
    ...
    try {
      ...
      if (res.ok) setEditSectionImage(data.fileUrl);
  ...
  const handleEditSectionModelFileChange = async (e) => {
    ...
    try {
      const formData = new FormData();
      formData.append('file', file);
      formData.append('fileType', 'models');
      const res = await fetch('/api/uploadModel', { method: 'POST', body: formData });
      const data = await res.json();
      if (res.ok) setEditSectionModel(data.fileUrl);
      else throw new Error(data.error);
    } catch (err) {
      console.error('Model upload error:', err);
      alert('3D-Modell-Upload fehlgeschlagen');
    }
  };
  ...
  const handleEditSectionPdfFileChange = async (e) => {
    ...
    try {
      const formData = new FormData();
      formData.append('file', file);
      formData.append('fileType', 'pdfs');
      const res = await fetch('/api/upload', { method: 'POST', body: formData });
      const data = await res.json();
      if (res.ok) setEditSectionPdf(data.fileUrl);
      else throw new Error(data.error);
    } catch (err) {
            ...
            <div>
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: '6px' }}>
                  <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6"></path>
